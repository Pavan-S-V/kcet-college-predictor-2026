...
import { ArrowLeft, Loader2, Mic, MicOff, Send, Volume2, VolumeX, GraduationCap } from "lucide-react";
...
function ChatThread() {
  ...
  useEffect(() => {
    supabase.from("chat_messages").select("id,role,content").eq("thread_id", threadId).order("created_at")
      .then(({ data, error }) => {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-9rem)] max-w-3xl flex-col px-4 py-4 sm:px-6">
      <div className="mb-3 flex items-center gap-2">
        <Link to="/chat"><Button variant="ghost" size="sm"><ArrowLeft className="h-4 w-4 mr-1" /> Threads</Button></Link>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto rounded-2xl border border-border bg-surface p-4">
        {messages.length === 0 && !sending && (
          <div className="grid h-full place-items-center text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">e.g. "What documents do I need for round 1?"</p>
            </div>
          </div>
        )}
        <div className="space-y-4">
          ...
          {sending && (
            <div className="flex justify-start">
              <div className="rounded-2xl bg-muted px-4 py-2.5 text-sm text-muted-foreground">Thinking<span className="animate-pulse">...</span></div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-3 rounded-2xl border border-border bg-surface p-2">
        <div className="flex items-end gap-2">
          <Textarea ref={taRef} value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Ask about colleges, branches, cutoffs..." rows={2}
